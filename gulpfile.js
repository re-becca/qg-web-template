...
const buildMonth =
  buildDate.getMonth() + 1 < 10 ? '0' + (buildDate.getMonth() + 1) : buildDate.getMonth();
const buildDay = buildDate.getDate() < 10 ? '0' + buildDate.getDate() : buildDate.getDay();
const buildHours =
  buildDate.getHours() + 1 < 10 ? '0' + (buildDate.getHours() + 1) : buildDate.getHours();
const buildMinutes =
  buildDate.getMinutes() < 10 ? '0' + buildDate.getMinutes() : buildDate.getMinutes();
const banner =
  '/*! SWE' +
  ' ' +
  pjson.version +
...
gulp.task('clean-redundant-release', cb => {
  return del(
    [
      `${config.basepath.release}/template-cdn/assets`,
      `${config.basepath.release}/template-local/assets/includes-local`,
    ],
    cb
  );
});
...
gulp.task(
  'template-pages',
  require('./gulp/build-tasks/template-pages')(
    gulp,
    plugins,
    config,
    'template-pages',
...
gulp.task(
  'template-pages-docs',
  require('./gulp/build-tasks/template-pages')(gulp, plugins, config, 'docs', 'docs')
);
gulp.task(
...
gulp.task(
  'other-assets-root',
  require('./gulp/build-tasks/other-assets')(gulp, plugins, config, es, assetDests[0])
);
gulp.task(
  'other-assets-docs',
  require('./gulp/build-tasks/other-assets')(gulp, plugins, config, es, assetDests[1])
);
...
gulp.task(
  ...
  require('./gulp/build-tasks/assets-includes')(
    gulp,
    plugins,
    config,
    'assets/includes-local',
...
gulp.task(
  'assets-includes-cdn',
  require('./gulp/build-tasks/assets-includes')(gulp, plugins, config, 'assets/includes-cdn')
);
...
gulp.task('watch:project', function() {
  gulp.watch(
    ...
    ['assets-includes-local', 'assets-includes-docs']
  );
  gulp.watch([`${config.basepath.src}/assets/_project/**/*.scss`], ['scss']);
  gulp.watch(`${config.basepath.src}/assets/_project/_blocks/**/*.js`, { verbose: true }, [
    'js',
    'test:eslint',
  ]);
  gulp.watch(`${config.basepath.src}/assets/_project/lib/**/*.js`, { verbose: true }, [
    'other-assets',
  ]);
  gulp.watch([`${config.basepath.src}/assets/_project/images/**/*`], ['other-assets']);
  gulp.watch(
    [`${config.basepath.src}/template-pages/**/*`],
    ['template-pages', 'template-pages-to-docs']
  );
...
gulp.task('watch:docs', function() {
  gulp.watch(
    [config.basepath.src + '/docs/**/*.html'],
    ['template-pages-docs', 'assets-includes-docs', 'template-pages-to-docs']
  );
  gulp.on('stop', () => {
    return plugins.shell([
      'node gulp/build-tasks/node-docs-flatten.js && gulp clean-redundant-build',
    ]);
  });
});
...
gulp.task(
  'release-other-files',
  require('./gulp/release-tasks/other-files')(gulp, plugins, config)
);
gulp.task(
  'replace-links',
  require('./gulp/release-tasks/replace-links')(gulp, plugins, es, config)
);
gulp.task(
  'release-files',
  require('./gulp/release-tasks/files')(gulp, plugins, config, es, webpack, path, banner)
);

gulp.task('release', cb => {
  return runSequence(['release-files', 'scss-src', 'release-other-files'], cb);
});
...
gulp.task(
  'serve',
  require('./gulp/build-tasks/serve')(gulp, plugins, connect, connectssi, argv, path, randomPort)
);

/* TEST TASKS */
gulp.task(
  'test:eslint',
  require('./gulp/test-tasks/lint')(gulp, plugins, config, fsPath, eslintReporter)
);
...
gulp.task(
  'wt-clone',
  require('./gulp/publish-tasks/git').clone(
    config.webTemplateRepo.url,
    config.webTemplateRepo.folder
  )
);
gulp.task(
  'wt-sync',
  require('./gulp/publish-tasks/git').sync(config.basepath.release, config.webTemplateRepo.folder, [
    'package.json',
  ])
);
gulp.task(
  'wt-updateVersion',
  require('./gulp/publish-tasks/git').updateVersion(
    config.webTemplateRepo.folder,
    pjson['wt-version']
  )
);
...
gulp.task(
  'wt-commit',
  require('./gulp/publish-tasks/git').commit(config.webTemplateRepo.folder, pjson['wt-version'])
);
gulp.task(
  'wt-tag',
  require('./gulp/publish-tasks/git').tag(config.webTemplateRepo.folder, pjson['wt-version'])
);
...
gulp.task(
  'cdn-clone',
  require('./gulp/publish-tasks/git').clone(config.staticCdnRepo.url, config.staticCdnRepo.folder)
);
...
gulp.task(
  'cdn-commit',
  require('./gulp/publish-tasks/git').commit(config.staticCdnRepo.folder, pjson.version)
);
