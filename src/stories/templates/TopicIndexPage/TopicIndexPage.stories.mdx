import { Canvas, Meta, Story } from "@storybook/addon-docs";

import { getCanvasMobileProps, getStoryMobileParameters, getStoryMobileHeight } from "../../helpers";

import Default from "../../../template-pages/topic-index-page.html";
import Mobile from "../../../template-pages/topic-index-page.html";
import New from "../../../template-pages/new-topic-index-page.html";
import Nocards from "../../../template-pages/new-topic-index-page-nocards.html";
import Noaside from "../../../template-pages/new-topic-index-page-noaside.html";
import Nosublinks from "../../../template-pages/new-topic-index-page-nosublinks.html";

<Meta title="Templates/TopicIndexPage" />

# Topic Index Page

## Default

<Canvas withSource="close">
  <Story name="Default">{() => Default}</Story>
</Canvas>

## Mobile

<Canvas withSource="none" {...getCanvasMobileProps()}>
  <Story name="Mobile" parameters={getStoryMobileParameters()} height={getStoryMobileHeight()}>{() => Mobile}</Story>
</Canvas>


<Canvas withSource="close">
  <Story name="New">{() => New}</Story>
</Canvas>

<Canvas withSource="close">
  <Story name="Nocards">{() => Nocards}</Story>
</Canvas>

<Canvas withSource="close">
  <Story name="Noaside">{() => Noaside}</Story>
</Canvas>

<Canvas withSource="close">
  <Story name="Nosublinks">{() => Nosublinks}</Story>
</Canvas>
